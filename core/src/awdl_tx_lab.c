...
#define AWDL_TX_LAB_DATA_INTERVAL 3U
#define AWDL_TX_LAB_DATA_LIMIT 40U
...
static void lab_tx_task(void *argument)
{
    ...
    while ((uint64_t)(esp_timer_get_time() - started_us) <
        ...
        if (attempted % AWDL_TX_LAB_DATA_INTERVAL == 0U &&
            data_attempted + echo_attempted < AWDL_TX_LAB_DATA_LIMIT) {
            const bool send_neighbor_solicitation =
                data_attempted == echo_attempted;
